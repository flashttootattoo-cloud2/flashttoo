...
export default function ReclamarEstudio() {
  ...
  async function handleActivate() {
    ...
    const r = await fetch(`/api/studios/${id}/claim`, {
    ...
    if (authError || !auth.session || !studio) {
      setError('El perfil se activó pero no se pudo iniciar sesión — entrá manualmente desde la app')
      setSubmitting(false)
      return
    }

    // Recupera el slug real (puede haber cambiado si "estudio" chocaba con otro)
    const fresh = await fetch(`/api/studios/${id}/preview`).then(r => r.json()).catch(() => null)

    const session = {
      slug: fresh?.slug ?? undefined,
      name: studio.name,
    ...
    try { localStorage.setItem('flashttoo_studio_session', JSON.stringify(session)) } catch {}
    ...
    <div style={{ minHeight: '100dvh', background: '#050505', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: 20, fontFamily: '-apple-system, BlinkMacSystemFont, "Segoe UI", sans-serif' }}>
      ...
      {loading ? (
        ...
        <div style={{ width: '100%', maxWidth: 340, animation: 'fadeIn 0.45s ease' }}>
          {/* Misma estructura visual que el perfil real de estudio (components/StudioPanel.tsx) */}
          <div style={{ background: '#111', borderRadius: 20, border: '1px solid rgba(255,255,255,0.08)', overflow: 'hidden', boxShadow: '0 20px 60px rgba(0,0,0,0.6)' }}>
            <div style={{ position: 'relative', width: '100%', paddingBottom: '100%' }}>
              {studio.logo_url ? (
                // eslint-disable-next-line @next/next/no-img-element
                <img src={studio.logo_url} alt={studio.name} style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', objectFit: 'cover' }} />
              ) : (
                <div style={{ position: 'absolute', inset: 0, display: 'flex', alignItems: 'center', justifyContent: 'center', background: '#1a1a1a' }}>
                  <span style={{ fontSize: 48, fontWeight: 800, color: 'rgba(255,255,255,0.15)' }}>{studio.name.slice(0, 2).toUpperCase()}</span>
                </div>
              )}
              <div style={{ position: 'absolute', inset: 0, background: 'linear-gradient(to top, #111 0%, rgba(0,0,0,0.5) 50%, transparent 100%)' }} />
              <div style={{ position: 'absolute', top: 14, left: 16, fontSize: 9, fontWeight: 800, letterSpacing: '0.1em', textTransform: 'uppercase', padding: '3px 9px', borderRadius: 5, background: 'rgba(0,0,0,0.6)', border: '1px solid rgba(239,255,66,0.3)', color: '#efff42' }}>
                Estudio
              </div>
              <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, padding: 20 }}>
                <h2 style={{ fontSize: 26, fontWeight: 700, lineHeight: 1.1, color: '#fff', overflowWrap: 'break-word' }}>{studio.name}</h2>
            ...
            {studio.description && (
              <div style={{ padding: '14px 20px 20px' }}>
                <p style={{ fontSize: 13, color: 'rgba(255,255,255,0.5)', lineHeight: 1.7 }}>{studio.description}</p>
              </div>
            )}
          </div>

          <p style={{ marginTop: 16, fontSize: 12, color: 'rgba(255,255,255,0.45)', textAlign: 'center', lineHeight: 1.6 }}>
            Así se ve tu estudio en Flashttoo. Activalo para que quede visible y puedas editarlo cuando quieras.
          </p>
