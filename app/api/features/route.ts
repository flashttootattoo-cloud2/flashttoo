...
export async function GET() {
  ...
  const { data } = await sb.from('settings').select('key,value').in('key', ['artist_gallery_enabled', 'events_country_filter', 'registration_open', 'verification_instagram', 'verification_whatsapp'])
