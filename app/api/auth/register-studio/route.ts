...
export async function POST(req: NextRequest) {
  ...
  const { data: invite } = await sb().from('studio_invites').select('id, used_by_studio_id').eq('token', invite_token).single()
  if (!invite || invite.used_by_studio_id) return NextResponse.json({ error: 'La invitación no es válida o ya fue usada' }, { status: 400 })

  const { data: existing } = await sb().from('studios').select('id').eq('auth_email', email.toLowerCase()).limit(1)
  if (existing?.length) return NextResponse.json({ error: 'Ya existe un estudio con ese mail' }, { status: 400 })
  ...
  const authUser = await (async () => {
    const result = await sb().auth.admin.createUser({
      ...
      password,
      email_confirm: true,
      user_metadata: { account_type: 'studio' },
    })
  ...
  const profileUrl = `${process.env.NEXT_PUBLIC_SITE_URL || 'https://flashttoo.com'}/completar-estudio?user_id=${authUser.id}&email=${encodeURIComponent(email.toLowerCase())}&invite=${invite_token}`

  const resend = new Resend(process.env.RESEND_API_KEY)
  await resend.emails.send({
    from: `Flashttoo <${process.env.RESEND_FROM || 'info@flashttoo.com'}>`,
    to: email.toLowerCase(),
    subject: 'Completá el perfil de tu estudio en Flashttoo',
    html: `
      <div style="background:#000;padding:40px 24px;font-family:sans-serif;max-width:480px;margin:0 auto">
        ...
        <p style="color:rgba(255,255,255,0.5);font-size:14px;line-height:1.7;margin:0 0 32px">
          Completá el perfil de tu estudio para que sea visible en Flashttoo.
        </p>
        <a href="${profileUrl}"
