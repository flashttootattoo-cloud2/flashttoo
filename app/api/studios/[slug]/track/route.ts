...
export async function POST(req: NextRequest, { params }: { params: Promise<{ slug: string }> }) {
  ...
  if (type === 'like' || type === 'unlike') {
    const { data } = await sb.from('studios').select('id, likes').eq('slug', slug).single()
    if (data) {
      const cur = (data.likes as number) ?? 0
      await sb.from('studios').update({ likes: type === 'like' ? cur + 1 : Math.max(0, cur - 1) }).eq('id', data.id)
    }
